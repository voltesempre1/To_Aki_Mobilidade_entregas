const Map<String, String> enUS = {
  'please_wait': "Please wait",
  "Upload all required documents.": "Upload all required documents.",
  "Do you want to start this Ride?": "Do you want to start this Ride?",
  "Ask the customer for an OTP so that you can start this ride":
      "Ask the customer for an OTP so that you can start this ride",
  "Cancel": "Cancel",
  "Ask for OTP": "Ask for OTP",
  "Ride Detail": "Ride Detail",
  "Pickup": "Pickup",
  "Complete Ride": "Complete Ride",
  "Payment of this ride is Remaining From Customer": "Payment of this ride is Remaining From Customer",
  "Confirm Cash Payment": "Confirm Cash Payment",
  "Are you sure you want complete the  ride with a cash payment?":
      "Are you sure you want complete the  ride with a cash payment?",
  "Complete": "Complete",
  "Track Ride": "Track Ride",
  "Ride Status": "Ride Status",
  "Cab Details": "Cab Details",
  "Payment is Completed": "Payment is Completed",
  "Payment is Pending": "Payment is Pending",
  "Ride Details": "Ride Details",
  "Date": "Date",
  "Time": "Time",
  "Distance": "Distance",
  "Customer Details": "Customer Details",
  "Payment Method": "Payment Method",
  "No conversion found": "No conversion found",
  "Type your message here...": "Type your message here...",
  "Edit Profile": "Edit Profile",
  "Name": "Name",
  "Enter Name": "Enter Name",
  "Email": "Email",
  "Enter Email": "Enter Email",
  "Enter your Phone Number": "Enter your Phone Number",
  "Gender": "Gender",
  "Male": "Male",
  "Female": "Female",
  "Save": "Save",
  "Please Select": "Please Select",
  "Camera": "Camera",
  "Gallery": "Gallery",
  "MyTaxi": "MyTaxi",
  "verify_OTP": "Verify OTP",
  "Total Earnings": "Total Earnings",
  "Total Rides": "Total Rides",
  "New Ride": "New Ride",
  "Ongoing Ride": "Ongoing Ride",
  "You're Now Offline": "You're Now Offline",
  "Please change your status to online to access all features. When offline, you won't be able to access any functionalities.":
      "Please change your status to online to access all features. When offline, you won't be able to access any functionalities.",
  "Go Online": "Go Online",
  "Customer Reviews": "Customer Reviews",
  "View all": "View all",
  "Welcome to Tô aki Mobilidade": "Welcome to Tô aki Mobilidade",
  "Your reliable partner for swift and convenient rides. Experience hassle-free transportation at your fingertips.":
      "Your reliable partner for swift and convenient rides. Experience hassle-free transportation at your fingertips.",
  "Fast and Reliable": "Fast and Reliable",
  "MyTaxi ensures prompt and reliable rides, getting you to your destination with speed and efficiency.":
      "MyTaxi ensures prompt and reliable rides, getting you to your destination with speed and efficiency.",
  "Skip": "Skip",
  "Seamless User Experience": "Seamless User Experience",
  "Enjoy a user-friendly interface, easy bookings, and secure payments with Tô aki Mobilidade. Your journey, your way.":
      "Enjoy a user-friendly interface, easy bookings, and secure payments with Tô aki Mobilidade. Your journey, your way.",
  "Get Started": "Get Started",
  "Language": "Language",
  "Login": "Login",
  "Please login to continue": "Please login to continue",
  "Send OTP": "Send OTP",
  "Please enter a valid number": "Please enter a valid number",
  "Continue with": "Continue with",
  "Apple": "Apple",
  "Google": "Google",
  "My Rides": "My Rides",
  "Ongoing": "Ongoing",
  "Completed": "Completed",
  "Cancelled": "Cancelled",
  "Rejected": "Rejected",
  "My Wallet": "My Wallet",
  "Total Balance": "Total Balance",
  "title": "title",
  "Add Money": "Add Money",
  "Transaction History": "Transaction History",
  "Add": "Add",
  "Please Enter minimum amount of": "Please Enter minimum amount of",
  "Please enter amount": "Please enter amount",
  "Please select payment type": "Please select payment type",
  "Notifications": "Notifications",
  "Enter OTP to Start Ride": "Enter OTP to Start Ride",
  "Please enter the OTP which provided by customer to confirm your acceptance of the ride request.":
      "Please enter the OTP which provided by customer to confirm your acceptance of the ride request.",
  "Submit": "Submit",
  "Confirm Ride Request": "Confirm Ride Request",
  "Are you sure you want to accept this ride request? Once confirmed, you will be directed to the next step to proceed with the ride.":
      "Are you sure you want to accept this ride request? Once confirmed, you will be directed to the next step to proceed with the ride.",
  "Confirm": "Confirm",
  "Please enter a valid OTP.": "Please enter a valid OTP.",
  "Welcome to": "Welcome to",
  "Enable location access to enjoy seamless ride experiences and reliable transportation services wherever you go.":
      "Enable location access to enjoy seamless ride experiences and reliable transportation services wherever you go.",
  "Allow Access": "Allow Access",
  "Reasons for Canceling Ride": "Reasons for Canceling Ride",
  "Continue": "Continue",
  "Your ride is successfully cancelled.": "Your ride is successfully cancelled.",
  "We hope to serve you better next time.": "We hope to serve you better next time.",
  "Back to Home": "Back to Home",
  "Home": "Home",
  "Something went wrong!": "Something went wrong!",
  "Type here...": "Type here...",
  "Create Account": "Create Account",
  "Create an account to start ride rides.": "Create an account to start ride.",
  "Enter Full Name": "Enter Full Name",
  "Full Name": "Full Name",
  "Email Address": "Email Address",
  "Enter Email Address": "Enter Email Address",
  "Phone Number": "Phone Number",
  "Enter Phone Number": "Enter Phone Number",
  "Sign Up": "Sign Up",
  "Vehicle Details": "Vehicle Details",
  "Select Vehicle Type": "Select Vehicle Type",
  "Vehicle Brand": "Vehicle Brand",
  "Select Vehicle Brand": "Select Vehicle Brand",
  "Vehicle Model": "Vehicle Model",
  "Select Vehicle Model": "Select Vehicle Model",
  "Vehicle Number": "Vehicle Number",
  "Select Vehicle Number": "Select Vehicle Number",
  "Please enter a valid details": "Please enter a valid details",
  "Upload": "Upload",
  "Front Side": "Front Side",
  "Browse": "Browse",
  "Back Side": "Back Side",
  "Upload clear pictures of both sides of": "Upload clear pictures of both sides of",
  "The uploaded image should be in .jpg, .png, or .pdf format.":
      "The uploaded image should be in .jpg, .png, or .pdf format.",
  "Date of Birth": "Date of Birth",
  "Enter Date of Birth": "Enter Date of Birth",
  "camera": "camera",
  "gallery": "gallery",
  "Verify your details": "Verify your details",
  "Check Status": "Check Status",
  "User disabled by administrator, Please contact to admin": "User disabled by administrator, Please contact to admin",
  "Upload Your Documents": "Upload Your Documents",
  "Securely upload required documents for identity verification and account authentication":
      "Securely upload required documents for identity verification and account authentication",
  "Add Your Vehicle Details": "Add Your Vehicle Details",
  "Verified": "Verified",
  "Not Verified": "Not Verified",
  "Verify Your Phone Number": "Verify Your Phone Number",
  "Enter  6-digit code sent to your mobile number to complete verification.":
      "Enter  6-digit code sent to your mobile number to complete verification.",
  "Did’t Receive a code ?": "Did’t Receive a code ?",
  "Resend Code": "Resend Code",
  "No Rides Found": "No Rides Found",
  "Your ride history is currently empty. Start your journey with Tô aki Mobilidade by riding your first ride now!":
      "Your ride history is currently empty. Start your journey with Tô aki Mobilidade by riding your first ride now!",
  "Pickup Point": "Pickup Point",
  "Dropout Point": "Dropout Point",
  "Marcado Pago Payment": "Marcado Pago Payment",
  "Cancel Payment": "Cancel Payment",
  "cancelPayment?": "cancelPayment?",
  "Exit": "Exit",
  "Continue Payment": "Continue Payment",
  "Payment": "Payment",
  "Services": "Services",
  "Failed to update status please try again": "Failed to update status please try again",
  "Online": "Online",
  "Document": "Document",
  "Rate Us": "Rate Us",
  "About": "About",
  "App Setting": "App Setting",
  "Light Mode": "Light Mode",
  "Logout": "Logout",
  "Are you sure you want to logout?": "Are you sure you want to logout?",
  "Log out": "Log out",
  "Terms & Condition": "Terms & Condition",
  "Privacy & Policy": "Privacy & Policy",
  "Cancel Ride": "Cancel Ride",
  "Are you sure you want cancel this ride?": "Are you sure you want cancel this ride?",
  "Ride cancelled successfully!": "Ride cancelled successfully!",
  "Your Ride is Rejected": "Your Ride is Rejected",
  "Accept": "Accept",
  "Ride accepted successfully!": "Ride accepted successfully!",
  "Your Ride is Accepted": "Your Ride is Accepted",
  "New": "New",
  "My Bank": "My Bank",
  "please_select": "Please Select",
  "No Customer review found": "No customer review found",
  "Support": "Support",
  "Create Ticket": "Create Ticket",
  "Search Intercity Ride": "Search Intercity Ride",
  "commission_fee_message":
      "A commission of @amount will be deducted from each ride payment for administrative purposes.",
  "Title": "Title",
  "Select Title": "Select Title",
  "Subject": "Subject",
  "This field required": "This field required",
  "Enter Subject": "Enter Subject",
  "Description": "Description",
  "Enter Description": "Enter Description",
  "choose Image": "choose Image",
  "Please Select Title": "Please Select Title",
  "Please Choose the Image": "Please Choose the Image",
  "Support Ticket Saved": "Support Ticket Save",
  "No Ticket Found": "No Ticket Found",
  "Note by Admin": "Note by Admin",
  "No Notes": "No Notes",
  "Your ride is completed....": "Your ride is completed....",
  "Recent Views": "Recent Views",
  "No Available Any Bid": "No Available Any Bid",
  "Closed": "Closed",
  "Active": "Active",
  "insufficient_wallet_balance":
      "You do not have sufficient wallet balance to accept the ride, as the minimum amount required is @amount.",
  "Ride Sharing": "Ride Sharing",
  "Price Details": "Price Details",
  "Amount": "Amount",
  "Discount": "Discount",
  "Total Amount": "Total Amount",
  "Add Bid": "Add Bid",
  "Confirm Intercity Ride Request": "Confirm Intercity Ride Request",
  "Intercity Ride accepted successfully!": "Intercity Ride accepted successfully!",
  "No booking details available.": "No booking details available.",
  "Enter Bid Amount": "Enter Bid Amount",
  "Bid": "Bid",
  "View Bid": "View Bid",
  "Parcel Ride Detail": "Parcel Ride Detail",
  "Dimension": "Dimension",
  "Parcel Image": "Parcel Image",
  "Reasons for Canceling Intercity Ride": "Reasons for Canceling Intercity Ride",
  "Your intercity ride is successfully cancelled.": "Your intercity ride is successfully cancelled.",
  "Intercity Ride Cancelled Successfully..": "Intercity Ride Cancelled Successfully..",
  "No Search Data": "No Search Data",
  "Select Date": "Select Date",
  "Cab Rides": "Cab Rides",
  "Intercity Rides": "Intercity Rides",
  "Parcel Rides": "Parcel Rides",
  "Ride Statement": "Ride Statement",
  "Download Ride Statement": "Download Ride Statement",
  "Select your preferred cab type, choose a date range and download your ride statement":
      "Select your preferred cab type, choose a date range and download your ride statement",
  "Select Ride Type": "Select Ride Type",
  "Select Star date to End Date": "Select Star date to End Date",
  "Download": "Download",
  "Select Ride Booking Date": "Select Ride Booking Date",
  "clear": "clear",
  "OK": "OK",
  "Delete account": "Delete account",
  "Your account will be deleted permanently. Your Data will not be Restored Again":
      "Your account will be deleted permanently. Your Data will not be Restored Again",
  "Account Deleted Successfully..": "Account Deleted Successfully..",
  "Alert!": "Alert!",
  "Would you like to cancel this payment?": "Would you like to cancel this payment?",
  "Continue to Paypal": "Continue to Paypal",
  "Tô Aki Mobilidade e Entregas": "Tô Aki Mobility and Deliveries",
  "Yes": "Yes",
  "Dark Mode": "Dark Mode",
  "System Mode": "System Mode",
  "Verify OTP": "Verify OTP",
  "Phone": "Phone",
  "Please enter your email": "Please enter your email",
  "Please enter a valid email": "Please enter a valid email",
  "Enter your email": "Enter your email",
  "Enter your password": "Enter your password",
  "Please enter your password": "Please enter your password",
  "password_min_length_login": "Password must be at least 6 characters",
  "Forgot Password?": "Forgot Password?",
  "Please fill all fields correctly": "Please fill all fields correctly",
  "Don't have an account?": "Don't have an account?",
  "Erro ao obter informações de rota": "Error getting route information",
  "Erro ao calcular rota. Por favor, tente novamente.": "Error calculating route. Please try again.",
  "Erro ao carregar a chave da API do Google Maps. Por favor, tente novamente mais tarde.":
      "Error loading Google Maps API key. Please try again later.",
  "Erro ao obter localização": "Error getting location",
  "Erro ao obter localização. Por favor, tente novamente.": "Error getting location. Please try again.",
  "Delete": "Delete",
  "Your Subscription": "Your Subscription",
  "Subscription Plan": "Subscription Plan",
  "Delete Account": "Delete Account",

  // Change Password Module Texts
  "Change Password": "Change Password",
  "Password Change Not Available": "Password Change Not Available",
  "You signed up using Google or Apple. Password changes are not available for social login accounts.":
      "You signed up using Google or Apple. Password changes are not available for social login accounts.",
  "Security": "Security",
  "Update your password to keep your account secure": "Update your password to keep your account secure",
  "Current password": "Current password",
  "New password": "New password",
  "Confirm new password": "Confirm new password",
  "Password requirements:": "Password requirements:",
  "• At least 6 characters": "• At least 6 characters",
  "• Different from current password": "• Different from current password",
  "Update Password": "Update Password",
  // "Please fill all fields correctly": "Please fill all fields correctly",

  // Change Password Controller Texts
  "User not logged in": "User not logged in",
  "Password updated successfully": "Password updated successfully",
  "Current password is incorrect": "Current password is incorrect",
  "New password is too weak": "New password is too weak",
  "Please login again before changing password": "Please login again before changing password",
  "User account has been disabled": "User account has been disabled",
  "User not found": "User not found",
  "Invalid email address": "Invalid email address",
  "Too many requests. Try again later": "Too many requests. Try again later",
  "Failed to update password": "Failed to update password",
  "Please enter your current password": "Please enter your current password",
  "Please enter a new password": "Please enter a new password",
  "password_min_length_change": "Password must be at least 6 characters",
  "New password must be different from current password": "New password must be different from current password",
  "Please confirm your new password": "Please confirm your new password",
  "Passwords do not match": "Passwords do not match",

  // Email Signup Module Texts
  "Please fill in the information to create your account": "Please fill in the information to create your account",
  "Confirm your password": "Confirm your password",
  "Already have an account? ": "Already have an account? ",
  "Please confirm your password": "Please confirm your password",
  "The password provided is too weak.": "The password provided is too weak.",
  "The account already exists for that email.": "The account already exists for that email.",
  "The email address is not valid.": "The email address is not valid.",
  "Email/password accounts are not enabled.": "Email/password accounts are not enabled.",
  "An error occurred during signup.": "An error occurred during signup.",

  // Login Module Texts
  "login_phone": "Phone",
  "login_enter_email": "Enter your email",
  "login_enter_password": "Enter your password",
  "login_forgot_password": "Forgot Password?",
  "login_no_account": "Don't have an account? ",
  "login_please_enter_email": "Please enter your email",
  "login_please_enter_valid_email": "Please enter a valid email",
  "login_please_enter_password": "Please enter your password",
  "No user found for this email.": "No user found for this email.",
  "Wrong password provided.": "Wrong password provided.",
  "Login failed.": "Login failed.",
  "Account not found. Please sign up first.": "Account not found. Please sign up first.",
  "something_went_wrong": "Something went wrong",

  // Forgot Password Module Texts
  "Enter your email address and we'll send you a password reset link":
      "Enter your email address and we'll send you a password reset link",
  "Send Reset Email": "Send Reset Email",
  "Remember your password? ": "Remember your password? ",
  "Back to Login": "Back to Login",
  "Check Your Email": "Check Your Email",
  "We sent a password reset link to": "We sent a password reset link to",
  "Next steps:": "Next steps:",
  "1. Check your email inbox": "1. Check your email inbox",
  "2. Click the reset password link": "2. Click the reset password link",
  "3. Create your new password": "3. Create your new password",
  "Resend Email": "Resend Email",
  "Sending email...": "Sending email...",
  "Password reset email sent successfully": "Password reset email sent successfully",
  "No user found for this email address": "No user found for this email address",
  "Too many requests. Please try again later": "Too many requests. Please try again later",
  "Network error. Please check your connection": "Network error. Please check your connection",
  "Email address is required": "Email address is required",
  "Failed to send reset email": "Failed to send reset email",

  // Document Verification Module Texts
  "user_disable_admin_contact": "User disabled by administrator, please contact admin",
  "invalid_phone_number": "Invalid phone number",
  "multiple_time_request": "Multiple requests in short period. Try again later",
};
