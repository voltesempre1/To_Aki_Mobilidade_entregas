...
class ParcelRideScreenController extends GetxController {
  ...
  Future<void> getArgument() async {
    ...
    if (argumentData != null) {
      parcelModel.value = argumentData['parcelModel'];

      log("---------------------------> ${parcelModel.value.toJson()}");
      FirebaseFirestore.instance.collection(CollectionName.parcelRide).doc(parcelModel.value.id).snapshots().listen((bookingEvent) {
        if (bookingEvent.data() != null) {
          log("---------------------------> Booking data received");
          ParcelModel updatedBooking = ParcelModel.fromJson(bookingEvent.data()!);
          parcelModel.value = updatedBooking;

          FirebaseFirestore.instance
              .collection(CollectionName.drivers)
              .doc(parcelModel.value.driverId)
              .snapshots()
            ...
            if (driverEvent.data() != null) {
              ...
              if (lastDriverLocation == null ||
                  ...
                  lastBookingStatus != parcelModel.value.bookingStatus) {
              ...
              lastBookingStatus = parcelModel.value.bookingStatus.toString();
  ...
  void getPolyline({required double? sourceLatitude,
